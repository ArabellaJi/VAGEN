...
# SYNC_ROOT uses local SSD (/tmp) to avoid NFS latency during checkpoint read/write.
# The final sync dir is archived to project storage on EXIT so the checkpoint is not lost.
# (SLURM_JOB_ID is not yet set at this point for variables used before JOB_TMP is defined;
#  SYNC_ROOT is set again below after JOB_TMP is established.)
...
archive_ray_logs() {
  ...
  # --- archive sglang sync dir (checkpoint + latest_step.txt) ---
  if [ -d "${SYNC_ROOT}" ] && [ -n "$(ls -A "${SYNC_ROOT}" 2>/dev/null)" ]; then
    mkdir -p "${SYNC_ARCHIVE_DIR}"
    # Copy metadata files only (txt, json) to avoid copying the full ~6GB model weights
