#!/usr/bin/env bash
# Run Sokoban eval with Qwen2.5-VL-3B via local SGLang server.
# Usage: bash eval_qwen25_vl_3b.sh [extra run_eval overrides...]
set -euo pipefail
...
SERVER_LOG="${LOG_DIR}/sglang_3b_server.log"
EVAL_LOG="${LOG_DIR}/sglang_3b_eval.log"
...
python -m vagen.evaluate.run_eval \
  --config "${SCRIPT_DIR}/config_3b.yaml" \
