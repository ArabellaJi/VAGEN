...
class RayPPOTrainer:
    ...
    def _maybe_prepare_sglang_disk_weight_sync(self):
        ...
        print(
            f"[TIMING] RayPPOTrainer: save_checkpoint START "
            f"global_step={self.global_steps} path={actor_local_path}"
        )
        t0 = time.time()
        self.actor_rollout_wg.save_checkpoint(actor_local_path, None, self.global_steps, max_ckpt_to_keep=2)
        elapsed = time.time() - t0
        print(f"[TIMING] RayPPOTrainer: save_checkpoint DONE in {elapsed:.1f}s")
        ...
        print(
            f"[TIMING] RayPPOTrainer: disk-sync checkpoint READY "
            f"global_step={self.global_steps} path={hf_model_dir}"
        )
